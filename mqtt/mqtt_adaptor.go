...
func (a *Adaptor) DoWithQOS(event string, qos int, f func(msg Message) []byte) (paho.Token, error) {
	...
	token := a.client.Subscribe(event, byte(qos), func(client paho.Client, msg paho.Message) {
		...
		fmt.Println("replayMsg = ", replayMsg)
		ret := a.Publish(pubTopic, replayMsg)
		fmt.Println("ret = ", ret)
	})
...
func (a *Adaptor) Do(event string, f func(msg Message) []byte) bool {
...
type MqttRoute struct {
	// Method  TopicMethod
	Topic   string
	Handler func(msg Message) []byte
}

// RegisterSubHandlers 注册订阅路由
func (a *Adaptor) AddSubHandlers(c []MqttRoute) {
	for _, r := range c {
