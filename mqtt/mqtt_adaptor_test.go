...
func initTestAdaptorWithAuth() *Adaptor {
	return NewAdaptorWithAuth("tcp://localhost:1883", "client", "fzw", "12345678")
}
...
func TestMqttAdaptorWithAuthPublishWhenConnected(t *testing.T) {
	a := initTestAdaptorWithAuth()
	_ = a.Connect()
	data := []byte("o")
	assert.True(t, a.Publish("/hello", data))
}
