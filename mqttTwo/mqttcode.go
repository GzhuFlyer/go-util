...
type Client struct {
	...
	locker        *sync.Mutex
	// 消息收到之后处理函数
	observer func(c *Client, msg *MessageTwo)
}
...
// 消费消息
func (client *Client) Subscribe(observer func(c *Client, msg *MessageTwo), qos byte, topics ...string) error {
	if len(topics) == 0 {
...
func M2() {
	...
	wg.Add(1)
	go func() {
		err := client.Subscribe(func(c *Client, msg *MessageTwo) {
			fmt.Printf("接收到消息: %+v \n", msg)
			wg.Done()
		}, 1, testtopic)
	...
	err = client.Publish(testtopic, 1, false, data)
	if err != nil {
		panic(err)
	}
